...
> 作者：苏剑林 · 科学空间 · 2019-04-10
>
> 原文：<https://spaces.ac.cn/archives/6540>
>
...
首先，新词发现部分，用到了我自己写的库[nlp zero](<https://spaces.ac.cn/archives/5597>)，基本思路是先分别对“比赛所给语料”、“自己爬的一部分百科百科语料”做新词发现，然后两者进行对比，就能找到一批“比赛所给语料”的特征词。
...
# 导出csv格式
pd.Series(idxs[:20000]).to_csv('result_1.csv', encoding='utf-8', header=None, index=None)</code></pre>
...
<pre><code class="language-python"> # nlp zero提供了良好的封装，可以直到导出一个分词器，词表是新词发现得到的词表。
...
word_size = 100
word2vec = Word2Vec(DW(), size=word_size, min_count=2, sg=1, negative=10)</code></pre>
...
<pre><code class="language-python">def is_good(w):
...
<pre><code class="language-python"># 种子词，在第一步得到的词表中的前面部分挑一挑即可，不需要特别准
...
pd.Series([i for i in idxs if len(i) &gt; 2][:10000]).to_csv('result_1_2.csv', encoding='utf-8', header=None, index=None)</code></pre>
