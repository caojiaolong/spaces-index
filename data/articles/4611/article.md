...
> 作者：苏剑林 · 科学空间 · 2017-10-13
>
> 原文：<https://spaces.ac.cn/archives/4611>
>
...
[![baidu_jingsai](<https://spaces.ac.cn/usr/uploads/2017/10/616835801.jpg>)](<https://spaces.ac.cn/usr/uploads/2017/10/616835801.jpg>)
...
比赛官网（随时有失效的可能）：[http://js\.baidu\.com](<http://js.baidu.com>)
...
<pre><code class="language-python">img_size = 299 #定义一些参数
nb_classes = 100
batch_size = 32
...
l2_loss = Lambda(lambda x: K.sum(K.square(x[0]-x[1][:,0]), 1, keepdims=True), name='l2')([feature,centers]) #定义center loss</code></pre>
...
<pre><code class="language-python">model_1 = Model(inputs=[input_image,input_target], outputs=[predict,l2_loss,auxiliary])
...
for i,j in train_txt.items(): #每个类中拿出10%作为验证集
    ...
    valid_data = valid_data.append(j[int(len(j)*train_frac):], ignore_index=True)</code></pre>
...
<pre><code class="language-python">#定义插值方式，当初将它定义为函数，本是希望随机使用不同的插值方式，这也是一种数据扩增的方式，但后来去掉了随机性。
def interp_way():
    return 'nearest'
...
<pre><code class="language-python">result_filename = '__test_result_2_2245.txt' #这是预测结果文件，迁移学习的时候用
...
def test_data_generator(): #测试集的generator，生成结果文件用
    ...
    for img_path in test_imgs:
        ...
        if len(x) == batch_size:
            ...
            x = []
    if x:
        yield preprocess_input(np.array(x))</code></pre>
...
<pre><code class="language-python">if __name__ == '__main__':
    ...
    test_result_2.to_csv('test_result_2.txt', index=None, header=None, sep='\t')</code></pre>
...
完整代码请点击：[https://github\.com/bojone/baidu\_dog\_classifier](<https://github.com/bojone/baidu_dog_classifier>)
