...
日常只运行 `uv run python scripts/update_all.py`：同一次文章页响应同时提供正文和元数据。已有快照用于离线修复，不需要再单独运行元数据抓取器。过程日志和临时文件在 `.cache/`，预览产物在 `build/preview/`。详见 [维护说明](../docs/guides/maintenance.md)。
