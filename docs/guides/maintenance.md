...
日常提取把通过完整校验的正文指纹与图片 URL 保存到 `.cache/ingestion/verified-bodies.json`。后续运行逐篇读取 Markdown、正文快照、快照记录和校验报告的 SHA-256；四个文件的字节和校验器代码、依赖版本均未变时，复用提取阶段的检查结果，避免重新解析全站公式。缓存缺失、损坏或内容变化时重新完整校验；到期复查、失败重试和撤回逻辑仍照常执行。缓存不延长原站检查时间。

Actions 跨运行缓存这份小型校验记录，以及 robots 规则和域名冷却记录；首次运行或校验器更新后需重新建立缓存。图片阶段继续校验图片文件，独立建站完全不使用这份提取缓存，仍重验所有正文、公式和图片哈希。运行缓存和临时构建不入库。完全不访问网络请使用 `--offline`。
