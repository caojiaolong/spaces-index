...
[文档导航](../README.md) · [本地预览](local-preview.md) · [正文校验与许可](mirror.md)
...
在线更新先用归档、`state.json`、元数据和小型图片重试清单生成待办列表，只处理新增、失败恢复、到期复查、撤回和未完成的图片任务。正常旧文章不读取 Markdown、正文快照或图片文件，也不逐篇计算哈希。没有新文章且没有待办时，Action 直接结束；仅归档标题或日期变化也会同步索引。显式全库检查使用 `--full-scan`，发布构建始终独立完整校验。
...
# 只检查归档并列出待办，不打开旧正文或构建网站
uv run python scripts/update_all.py --plan-only

# 显式检查全部已有正文和图片，并构建
uv run python scripts/update_all.py --full-scan
...
| `docs/README.md`、`docs/guides/` | 人工维护的文档导航、使用与维护指南 |
| `docs/topics/` | 自动生成的主题索引与详细元数据 |
| `web/` | 网站源码和本地公式排版资源 |
...
根 README 和 `docs/topics/` 由 `scripts/render_markdown.py` 自动更新；分类修正应修改规则或 `data/overrides.yaml`，不要直接编辑生成页。`docs/README.md` 与 `docs/guides/` 可直接维护。仅整理文档或重新渲染索引不需要访问原站。
...
Actions 先用部分克隆和稀疏检出取得脚本、索引、状态及重试清单，不下载旧正文、图片或 MathJax 资源。`--plan-only` 检查归档，有待办或归档变化才补全检出，再用 `--cached-archive --skip-build` 更新并检查内容变化；同次运行只获取一次归档。需要发布时只构建一次，没有变化时跳过更新、正文校验缓存的恢复/保存、构建和部署。代码 push 会补全检出后测试、构建和发布，不抓取原站。

仅图片失败、延期或检查时间变化会保存重试记录，不会重新部署；新增、移除或替换可用图片，以及正文、撤回配置变化仍会构建。`--skip-build` 不能与 `--serve` 同用，本地统一入口默认仍会构建。
...
只有进入待办的文章或显式全库检查会使用 `.cache/ingestion/verified-bodies.json`：读取该篇 Markdown、正文快照、快照记录和校验报告的 SHA-256；四个文件的字节和校验器代码、依赖版本均未变时，复用提取阶段的检查结果。缓存缺失、损坏或内容变化时重新完整校验。正常旧文章不需要这份缓存也能跳过。缓存不延长原站检查时间。

Actions 有待办时才恢复/保存这份小型校验记录，robots 规则和域名冷却记录另行复用。图片阶段只处理待办文章的图片；中断前未完成的图片任务由 `images_pending` 标记保留，失败清单继续遵守冷却和 Retry-After。独立建站完全不使用这份提取缓存，仍重验所有正文、公式和图片哈希。运行缓存和临时构建不入库。完全不访问网络请使用 `--offline`。

归档按文档顺序读取年份，并复用月份容器的文字，避免对每篇文章重复扫描整页。Action 汇总分别显示归档网络请求（含限速等待）、归档解析、正文处理、图片处理的耗时与正文缓存命中数。没有文件变化时跳过构建和部署；代码 push 仍测试、构建并发布代码改动。没有新文章也可能有到期复查、修复或撤回，需要按实际内容变化判断。
...
在线更新自动补齐已缓存正文中的缺失图片，无需重抓文章页。允许范围为科学空间主域及 `album`、`bbs` 子域，另受下文 `skip_image_hosts` 跳过配置约束；图片保存到各自文章目录，站外图片不下载。已缓存且哈希正确的图片跳过；文章到期复查时，关联图片使用 ETag/Last-Modified 复查。同次更新重复图片 URL 复用下载结果。下载失败独立记录，不让文章正文失效；普通失败冷却一天，404/410 冷却七天，Retry-After 严格遵守。临时复查失败保留旧副本，确认 404/410 后撤掉本地副本。离线模式只使用已有图片，不请求网络。
...
部分历史正文使用 Markdown 中的 HTML 保留复杂结构，并独立逐节点比对，避免转换时遗漏。`1023`、`116`、`49` 的原文公式定界符异常保持原样，页面提示并不伪造公式总数。旧式脚本与插件保留位置和原始信息但不执行；远程图片与媒体仍取决于原地址是否可用。校验和授权边界见 [正文校验与许可](mirror.md)，提取失败的诊断命令见 [正文提取与失败恢复](local-extraction.md)。
