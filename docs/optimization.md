...
- 2026-08-31 - [让炼丹更科学一些（八）：多阶段训练的学习率](https://spaces.ac.cn/archives/11879)
  ...
  - 系列：让炼丹更科学一些 #8
  - 系列主题：优化与训练
  - 小结摘录：本文从多阶段训练的角度重新审视了“无调度学习率”，通过将调度目标改为“每个阶段结束时都接近最优”，使得实践上更简化且更可行，进一步地，我们提炼出“等效步数”的概念，使得结论可能迁移到实践所用的优化器和Scaling Law上。
- 2026-09-05 - [让炼丹更科学一些（九）：经典自适应梯度算法](https://spaces.ac.cn/archives/11882)
  - 原站分类：数学研究
  - 原站标签：不等式、梯度、学习率、优化器、炼丹
  - 系列：让炼丹更科学一些 #9
  - 系列主题：优化与训练
  - 小结摘录：本文回顾了自适应梯度算法的奠基之作——AdaGrad，并沿着“收敛性分析→最小化上界→最优预条件矩阵”的路线，复现了它的完整推导。可以说，AdaGrad所提的“基于梯度二阶矩调节学习率”，几乎是所有后续的自适应学习率优化器共同的底层原理。
- 2026-09-14 - [让炼丹更科学一些（十）：单调性假设的拆与补](https://spaces.ac.cn/archives/11885)
  - 原站分类：数学研究
  - 原站标签：不等式、矩阵、学习率、优化器、炼丹
  - 系列：让炼丹更科学一些 #10
  - 系列主题：优化与训练
  - 小结摘录：本文在上一篇的收敛框架之上拆掉了两个“脚手架”：一是允许学习率$\\eta_t$动态变化，推导出相应的平均收敛和终点收敛结论，初步解释LR Decay对优化器的通用性；二是对于非单调的$\\boldsymbol{H}_t$，讨论了最小改动思想下的修正方案，以及推导了无需单调性假设的更一般的收敛结论。 总体来说，本文的信息密度较大，我们跳过了不少证明细节，因为关键的证明技巧在前面关于SGD的系列文章中已经详细给出，所以本文更多是直接给出关键的步骤与结果。
...
- 2026-08-17 - [流形上的最速下降：7. Stiefel的解析解](https://spaces.ac.cn/archives/11864)
  ...
  - 系列：流形上的最速下降 #7
  - 系列主题：优化与训练
  - 小结摘录：在这篇文章中，我们成功为“Muon + Stiefel”的流形最速下降找到解析解，为该问题划上一个圆满的句号。自此，不管是否方阵，正交约束下的Muon优化器都可解析计算，求解本身将不再是困难。
...
- 2026-08-23 - [动量的新理解：逼近特征层面的梯度下降](https://spaces.ac.cn/archives/11875)
  - 原站分类：信息时代
  - 原站标签：线性、梯度、优化器、muon
  - 小结摘录：本文从让“参数梯度下降”逼近“特征梯度下降”的思想出发，将动量重新诠释为一个在线回归问题的解，由此引出了一些相关工作。有趣的是，这条路线跟线性注意力从Vanilla到DeltaNet再到MesaNet的演变极其一致，两者看起来有颇多可以相互借鉴的地方。
