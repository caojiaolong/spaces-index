...
- **更快进入内容**：清晰字阶、曲面主视觉与精简文章列表，首页直接搜索，手机首屏即可看到最新文章。
- **系列书架**：14 种数学图案与按系列固定的随机配色，配合书名、书脊、阅读进度和可定位的章节目录，让长篇系列更容易辨认和接着读。
- **主题交汇**：依据共同文章与共同标签探索关联，点击即可查看两个主题共同收录的文章。
- **探索筛选更完整**：支持搜索、主题、标签、年份、难度、系列文章、非系列文章以及已读/未读组合筛选，筛选状态可以随 URL 分享；显示不下的小结可原地展开、收起。
...
- [文章检索](https://caojiaolong.github.io/spaces-index/#/explore)：按标题、标签、系列和小结快速搜索，组合主题、年份、系列状态与已读状态等筛选条件。
