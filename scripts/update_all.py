...
def update(args) -> dict:
    ...
    result = {"finished_at": now_iso(), "elapsed_seconds": round(time.monotonic() - started, 1),
              ...
              "readable_articles": len(catalog["mirrors"]), "audience": args.audience, "output": str(destination),
    ...
    result["images"] = {key.removeprefix("images_"): value for key, value in
                        read_json(ROOT / ".cache/ingestion/summary.json", {}).get("this_run", {}).items()
...
def main(argv=None) -> int:
    ...
    try:
        ...
        return 1 if result["ingestion_exit_code"] or result["failures"] or result["metadata_failures"] else 0
